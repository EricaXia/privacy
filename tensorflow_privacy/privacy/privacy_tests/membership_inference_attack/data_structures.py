...
def _is_last_dim_equal(arr1, arr1_name, arr2, arr2_name):
    ...
    if arr1 is not None and arr2 is not None and arr1.shape[-1] != arr2.shape[-1]:
        raise ValueError(
            '%s and %s should have the same number of features.'
            % (arr1_name, arr2_name)
        )
...
class AttackInputData:
    ...
    def validate(self):
        ...
        if (
            self.labels_train is None
            and self.loss_train is None
            and self.logits_train is None
            and self.entropy_train is None
        ):
            raise ValueError(
        ...
        _is_last_dim_equal(
            self.logits_train, 'logits_train', self.logits_test, 'logits_test'
        )
        _is_last_dim_equal(
            self.probs_train, 'probs_train', self.probs_test, 'probs_test'
        )
...
class SingleMembershipProbabilityResult:
    ...
    def attack_with_varied_thresholds(self, threshold_list):
        ...
        fpr, tpr, thresholds = metrics.roc_curve(
            np.concatenate(
                (
                    np.ones(len(self.train_membership_probs)),
        ...
        max_prob = max(
            self.train_membership_probs.max(), self.test_membership_probs.max()
        )
        ...
        return (
            np.array(meaningful_threshold_list),
...
class AttackResults:
    ...
    def summary(self, by_slices=False) -> str:
        ...
        summary.append(
            '  %s (with %d training and %d test examples) achieved an AUC of %.2f on'
            ' slice %s'
            % (
        ...
        if by_slices and len(slice_dict.keys()) > 1:
            for slice_str in slice_dict:
                ...
                summary.append(
                    '\nBest-performing attacks over slice: \"%s\"' % slice_str
                )
    ...
    def get_result_with_max_auc(self) -> SingleAttackResult:
        ...
        if min(aucs) < 0.4:
            print(
                'Suspiciously low AUC detected: %.2f. '
    ...
    def get_result_with_max_attacker_advantage(self) -> SingleAttackResult:
        ...
        return self.single_attack_results[
            np.argmax(
                [
                    result.get_attacker_advantage()
    ...
    def save(self, dirname):
        ...
        for i, attack_results in enumerate(self.attack_results_list):
            filepath = os.path.join(
                dirname, _get_attack_results_filename(attack_results, i)
            )
...
def _get_attack_results_filename(attack_results: AttackResults, index: int):
    ...
    if metadata is not None:
        return '%s_%s_epoch_%s.pickle' % (
            metadata.model_variant_label,
            index,
            metadata.epoch_num,
        )
